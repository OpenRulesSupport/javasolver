...
public class Warehouse extends JavaSolver {
	...
	public void define() {
		try {
			...
			open = csp.variableArray("assignVars",0, 1, nbSuppliers);
			...
			// totalHappinessVar= sum(cost) + sum(assignVars)*buildCost
	...
	public void saveSolution(Solution solution) {
		...
		str = new StringBuffer();
		str.append("assignVars: ");
		for (int i = 0; i < open.length; i++) {
			int value = solution.getValue("assignVars-"+i);
			str.append(value + " ");
		}
		csp.log(str.toString());
